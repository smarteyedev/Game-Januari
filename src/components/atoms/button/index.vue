<template>
    <component :is="tag" class="ui-button" :class="[
        `ui-button--${size}`,
        `ui-button--${color}`,
        `ui-button--${variant}`,
        {
    ...
    ]" :disabled="isDisabled || undefined" :style="genStyle" type="button">
        ...
        <slot name="prepend" class="ui-button__prepend">
            ...
            <UiAtomsIcon v-if="loading || icon" :name="loading ? 'uil-spinner' : icon" :width="getIconSize(size)"
                :height="getIconSize(size)" mode="svg" :class="{
        ...
        <slot name="append" class="ui-button__append">
            <UiAtomsIcon v-if="iconAppend" :name="iconAppend" :width="getIconSize(size)" :height="getIconSize(size)"
                mode="svg" />
        </slot>
    </component>
</template>
