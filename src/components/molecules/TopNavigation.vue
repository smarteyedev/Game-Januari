<template>
  <div class="flex flex-col justify-between gap-3.5">
    ...
    <div class="flex w-full gap-2.5" :class="{ 'justify-end': isIntro }">
      <UiButton v-if="!isIntro" :size="!isXs && !isSm ? 'md' : 'xs'" :icon-size="!isXs && !isSm ? 32 : 24" :square="true"
        icon="ri:home-5-fill" @click="goToHome" />
      <UiButton v-if="!isIntro" :size="!isXs && !isSm ? 'md' : 'xs'" variant="primary" text="View Summary" class="grow w-full"
