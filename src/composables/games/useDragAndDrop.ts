...
export function useDragAndDrop() {
  ...
  const totalSlots = computed(() => {
    return board.value.filter((part): part is { type: 'slot'; id: number; item: Blank | null } => 
      part.type === 'slot'
    ).length
  })
  ...
  function performDrop(dropSlotId: number, draggedItem: Blank, fromType: 'pool' | 'board', fromId?: number) {
    if (isLocked.value) return

    const currentSlotItem = slots.value[dropSlotId]

    if (currentSlotItem) {
      if (fromType === 'board' && fromId !== undefined) {
        slots.value[dropSlotId] = draggedItem
        slots.value[fromId] = currentSlotItem
      } else {
        slots.value[dropSlotId] = draggedItem
        items.value = items.value.filter((i) => i.id !== draggedItem.id)
